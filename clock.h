...
#define myDebug false    // 测试阶段为真，实际投入使用时需要改为false
    ...
    */
    ...
    // 基类的私有成员可通过基类的公有（保护）方法访问。

    // 公有派生是一种is-a的关系，即日期是时间就像苹果是水果一样
    ...
    public:
        ...
        explicit CalendarLite(const Clock &clo) : Clock(clo.hour(), clo.minute(), clo.second()),
        ...
        */
        ...
        /*
          ...
          3.友元函数不能是虚函数：友元函数不是类成员，只有类成员才能是虚函数（trick：让友元函数使用虚函数）
    ...
    CalendarLite::CalendarLite(calendar_t second) : Clock((clock_t)(second % (day_base * Clock::CLOCK_BASE() * Clock::CLOCK_BASE())))
    {
    ...
    {
        return ((day_ + month_ * month_base + year_ * year_base * month_base) * day_base + hour()) * CLOCK_BASE() * CLOCK_BASE() + minute() * CLOCK_BASE() + second();
    }
